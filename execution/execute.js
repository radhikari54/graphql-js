...
function execute(schema, documentAST, rootValue, contextValue, variableValues, operationName, logFn) {
  ...
  var context = buildExecutionContext(schema, documentAST, rootValue, contextValue, variableValues, operationName, logFn);
 ...
 */
function buildExecutionContext(schema, documentAST, rootValue, contextValue, rawVariableValues, operationName, logFn) {
  var errors = [];
...
function resolveField(exeContext, parentType, source, fieldASTs, path) {
  ...
  var logFn = exeContext.logFn || function () {};
  logFn(_logging.TAG.RESOLVER_START, { path: path }, info);
  ...
  if (isThenable(result)) {
    result = result.then((0, _logging.passThroughResultAndLog)(logFn, _logging.TAG.RESOLVER_END, { path: path }, info), function (reason) {
      logFn(_logging.TAG.RESOLVER_ERROR, {
        error: reason,
        path: path
      }, info);
      logFn(_logging.TAG.RESOLVER_END, { path: path });
    ...
    logFn(_logging.TAG.RESOLVER_END, { path: path }, info);
  }
...
function completeValueCatchingError(exeContext, returnType, fieldASTs, info, path, result) {
  var logFn = exeContext.logFn || function () {};
  logFn(_logging.TAG.SUBTREE_START, { path: path }, info);
  ...
  if (returnType instanceof _definition.GraphQLNonNull) {
    ...
    if (isThenable(completed)) {
      return completed.then((0, _logging.passThroughResultAndLog)(logFn, _logging.TAG.SUBTREE_END, { path: path }, info), function (reason) {
        logFn(_logging.TAG.SUBTREE_ERROR, {
          error: reason,
          path: path
        }, info);
        logFn(_logging.TAG.SUBTREE_END, { path: path }, info);
        return _promise2.default.reject(reason);
      });
    }
  ...
  try {
    var _completed = completeValue(exeContext, returnType, fieldASTs, info, path, result);
    if (isThenable(_completed)) {
      // If `completeValue` returned a rejected promise, log the rejection
      // error and resolve to null.
      // Note: we don't rely on a `catch` method, but we do expect "thenable"
      // to take a second callback for the error case.
      return _completed.then((0, _logging.passThroughResultAndLog)(logFn, _logging.TAG.SUBTREE_END, { path: path }, info), function (error) {
